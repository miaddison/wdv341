...
//{
	...
	if(isset($_POST["submit"]))
	{
		...
		$contact_more_products_msg = checkBoxOutput("contact_more_products");
        ...
        $emailBody .= '<p>Requested more information on products: '. $contact_more_products_msg.'</p>';
		...
		if($validForm){
			...
			if(mail($toEmail, $subject, $emailBody, $headers)){
				...
				$message = "Your information is not able to be submitted via email at this time. ";
			}
			try 
			{
				...
				$sql = "INSERT INTO miaddison_wdv.wdv_341_customer_contacts(";
				$sql .= "contact_name, ";
				$sql .= " contact_email, ";
				$sql .= " contact_reason, ";
				...
				$stmt->bindParam(':contactNewsletter', $contact_newsletter_msg);
				$stmt->bindParam(':contactMoreProducts', $contact_more_products_msg);
			...
			{
				$message .= "There has been a problem adding your information to the database. The system administrator has been contacted. Please try again later.";
...
<script>
	function resetForm(){
		...
		document.getElementById('contact_newsletter').checked = true;
		document.getElementById('contact_more_products').checked = true;
	}
</script>	
</head>
...
?>
        ...
        <p>Requested more information on products: <?php echo $contact_more_products_msg;?></p>
<?php
	}
